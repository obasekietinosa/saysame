...
- `GET /room/:roomId?lastUpdatedAt=<datetime>` returns the `RoomState` and will be polled to receive the up to date room information. It returns a 304 if the timestamp on the request is the same as the lastUpdatedAt for that room (skipping over the desrialisation and saving some CPU cycles). If we do not send a 304, the server relies on a lua script to make fetching from redis atomic (ie so that there's no data drifti between fetching the room metadata and the actual submissions)
- `GET /lobby/:playerId` check to see if player has been matched to room. Also functions as a keep alive for the lobby. If a player does not send a request to this endpoint within 5 seconds we consider them no longer in the lobby and they'll be cleaned up as part of the `POST /lobby` lifecycle. This endpoint returns a 404 if the player is not found in the lobby (either because the keep-alive expired and it was cleaned up or while being queued to play a failure happened). Upon 404, the client should re-register the player in the lobby via `POST /lobby`
